# preprocess the Peripheral Oxygen Saturation (SpO₂) data
import os
import pandas as pd
import numpy as np
...
def plot_raw_signals(file_path, n_sessions=3):
    ...
    plt.tight_layout()
    plt.show()
example_file = os.path.join(Spo2_data_dir, os.listdir(Spo2_data_dir)[0])
...
all_segments = []
for filename in os.listdir(Spo2_data_dir):
    if filename.endswith('.csv'):
        filepath = os.path.join(Spo2_data_dir, filename)
...
plt.xlabel('Hz'); plt.ylabel('PSD'); plt.title('Spo2 PSD by class'); plt.legend(); plt.tight_layout(); plt.show()

# ---------------------

output_path = f'preprocessed_spo2_segments_{segment_length}.csv'
