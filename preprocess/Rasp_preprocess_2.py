# preprocess the Respiratory (RESP) data
import os
import pandas as pd
import numpy as np
...
def plot_raw_signals(file_path, n_sessions=3):
    ...
    plt.tight_layout()
    plt.show()
example_file = os.path.join(Resp_data_dir, os.listdir(Resp_data_dir)[0])
...
all_segments = []
for filename in os.listdir(Resp_data_dir):
    if filename.endswith('.csv'):
        filepath = os.path.join(Resp_data_dir, filename)
...
plt.xlabel('Hz'); plt.ylabel('PSD'); plt.title('RASP PSD by class'); plt.legend(); plt.tight_layout(); plt.show()

# ---------------------
output_path = f'preprocessed_rasp_segments_analysis_{segment_length}.csv'
