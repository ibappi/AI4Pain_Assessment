...
def extract_features_from_signal(signal):
    ...
    features['peak_mean_height'] = np.mean(signal[peaks]) if len(peaks) > 0 else 0
...
def preprocess_eda_folder(data_dir):
    all_features = []
    ...
    for filename in os.listdir(data_dir):
        if filename.endswith('.csv'):
            ...
            for col in df.columns:
                ...
                if len(signal) < 10:
                    continue  # skip short/noisy segments
...
if __name__ == '__main__':
    features_df = preprocess_eda_folder(EDA_DATA_DIR)
    features_df.to_csv(OUTPUT_FILE, index=False)
    print(f"Done. Extracted features saved to: {OUTPUT_FILE}")
